...
public class ComunidadeDAO {
    ...
    // Cria um grupo e já adiciona o criador como ADMIN
    ...
    // Lista só os grupos dos quais o usuário é membro
    public List<ComunidadeVO> listarDoUsuario(int usuarioId) throws Exception {
        String sql = "SELECT c.id, c.nome, c.descricao, c.criador_id, mc.papel " +
    ...
    // Lista membros de um grupo (para mostrar na tela do grupo)
    public List<UsuarioVO> listarMembros(int comunidadeId) throws Exception {
        String sql = "SELECT u.id, u.nome, u.foto_perfil, mc.papel " +
    ...
    public List<java.util.Map<String, Object>> listarConvitesPendentes(int convidadoId) throws Exception {
        String sql = "SELECT cv.id, c.nome AS grupo_nome, u.nome AS quem_convidou " +
                     "FROM convites cv " +
                     "JOIN comunidades c ON cv.comunidade_id = c.id " +
    ...
    public ComunidadeVO buscarPorId(int id) throws Exception {
        ...
        try (Connection conn = ConexaoDB.conectar();
            ...
            stmt.setInt(1, id);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return new ComunidadeVO(rs.getInt("id"), rs.getString("nome"), rs.getString("descricao"));
            }
        }
        return null;
    }
