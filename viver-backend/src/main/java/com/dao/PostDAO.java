...
public class PostDAO {
    ...
    public PostVO salvar(PostVO post) throws Exception {
        ...
        try (Connection conn = ConexaoDB.conectar();
            ...
            stmt.setString(2, post.getImagem());   // pode ser null
    ...
    private List<PostVO> listar(int usuarioLogadoId, boolean soSeguidos) throws Exception {
        String filtro = soSeguidos
            ? "JOIN seguidores seg ON p.usuario_id = seg.seguido_id AND seg.seguidor_id = " + usuarioLogadoId
            : "";
        ...
        String sql = "SELECT p.id, p.conteudo, p.imagem, p.data_criacao, p.destino_tipo, " +
                     ...
                     "FROM postagens p JOIN usuarios u ON p.usuario_id=u.id " + filtro +
    ...
    public boolean alternarCurtida(int usuarioId, int postId) throws Exception {
        ...
        try (Connection conn = ConexaoDB.conectar();
            ...
            try (ResultSet rs = s.executeQuery()) {
                if (rs.next()) {
                    try (PreparedStatement d = conn.prepareStatement("DELETE FROM curtidas WHERE usuario_id=? AND post_id=?")) {
                        d.setInt(1, usuarioId); d.setInt(2, postId); d.executeUpdate();
                    }
                    return false;
                } else {
                    try (PreparedStatement i = conn.prepareStatement("INSERT INTO curtidas (usuario_id, post_id) VALUES (?,?)")) {
